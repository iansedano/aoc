...
def part_1(parsed_input):
    ...
    def get_next_possible_states(node):
        ...
        turns = (
            ...
            if (node.position, hd) not in queue.visited and node.position not in walls
        )

        if (
            step := (node.position + node.heading, node.heading)
        ) in queue.visited or step[0] in walls:
            return turns
