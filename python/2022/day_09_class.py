...
class Rope:
    def __init__(self, length):
        self.body = [Vec(0, 0) for _ in range(length)]
        self.positions = {(self.body[-1].x, self.body[-1].y)}

    def move(self, direction):
        self.body[0] = self.body[0] + DIRECTIONS[direction]
        for i in range(1, len(self.body)):
            self.body[i] = Rope.new_tail_position(
                self.body[i - 1], self.body[i]
            )
        self.positions.add((self.body[-1].x, self.body[-1].y))
    ...
    def visualize(self):
        ...
        for y in range(*y_range):
            for x in range(*x_range):
                if Vec(x, y) == self.body[0]:
                    print("H", end="")
                elif Vec(x, y) in self.body:
                    print(self.body.index(Vec(x, y)) + 1, end="")
                elif Vec(x, y) in self.positions:
                    print("#", end="")
                else:
                    print(".", end="")
            print()
    ...
    def __str__(self):
        return f"head : {self.body[0]}  |  tail : {self.body[1:]}"
